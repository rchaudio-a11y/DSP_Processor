...
#Region "Constructor"
        ...
        ''' </summary>
        ''' <param name="deviceInfo">Device to use</param>
        ''' <param name="sampleRate">Desired sample rate (will use device's closest supported rate)</param>
        ''' <param name="channels">Desired channel count (will use device's actual channels if different)</param>
        ''' <param name="bitsPerSample">Desired bit depth</param>
        ...
        Public Sub New(deviceInfo As DeviceInfo, sampleRate As Integer, channels As Integer, bitsPerSample As Integer, Optional bufferMs As Integer = 30)
            ...
            ' Use device's ACTUAL channel count if available
            _channels = If(deviceInfo.MaxChannels > 0, deviceInfo.MaxChannels, channels)
            
            ' Use requested sample rate (WaveIn will fail gracefully if unsupported)
            _sampleRate = sampleRate
            
            ' Use requested bit depth
            _bitsPerSample = bitsPerSample
            
            _bufferMs = bufferMs

            Utils.Logger.Instance.Info($"WaveInEngine created: Device={deviceInfo.Name}, Channels={_channels}, Rate={_sampleRate}Hz, Bits={_bitsPerSample}, Buffer={_bufferMs}ms", "WaveInEngine")
        End Sub
