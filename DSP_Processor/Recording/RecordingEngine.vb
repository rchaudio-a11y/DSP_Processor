...
Namespace Recording
    ...
    Public Class RecordingEngine
        ...
        ''' <summary>
        ''' Process audio buffer from callback (REAL-TIME - no queue polling!)
        ''' This is called directly from the audio driver callback with the buffer
        ''' </summary>
        Public Sub ProcessBuffer(buffer As Byte(), bytesRecorded As Integer)
            ...
            If _writeQueue.Count < MaxQueueDepth Then
                ...
                Dim writeBuffer(bytesRecorded - 1) As Byte
                Array.Copy(buffer, writeBuffer, bytesRecorded)
            ...
            If bytesRecorded = buffer.Length Then
                lastProcessedBuffer = buffer
            Else
                ' Partial buffer - copy only valid data
                ReDim lastProcessedBuffer(bytesRecorded - 1)
                Array.Copy(buffer, lastProcessedBuffer, bytesRecorded)
            End If
        ...
        ''' <summary>
        ''' DEPRECATED: Process using queue polling (legacy timer-driven mode)
        ''' Use ProcessBuffer() instead for glitch-free callback-driven recording
        ''' </summary>
        Public Sub Process()
